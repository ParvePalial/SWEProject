...
\begin{itemize}
    ...
    \item \textbf{UC4/UC5 - Post Items}: Allows reporting lost or found items. Found items are marked pending until an \textit{«include»} Admin Verification step is completed.
\end{itemize}

\subsection{Entity Relationship (ER) Model}
The database schema involves three primary entities:
\begin{enumerate}
    \item \textbf{User}: Stores credentials (hashed passwords), roles, and security questions.
    \item \textbf{Item}: Stores details for both Lost and Found entries, unifying them with a \texttt{type} discriminator and a \texttt{status} flag.
    \item \textbf{Claim}: Associates a User with a Found Item, storing proof of ownership details and approval status.
\end{enumerate}

\subsection{System Flow}
The application flow enforces security constraints: Guest users are restricted to the Search interface. Upon logging in, users enter a central Dashboard branching out into Item Posting, Claim Submission, and (if authorized) the Administrative Review portal.
